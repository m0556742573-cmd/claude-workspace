...
- `supabase-selfhost/seeds/` — נתוני ייחוס ובדיקה, נפרד מהסכימה:
  ...
  - `9004_obligation_template_scenarios.sql` — 4 רשויות ו-8 תבניות חובה שמכסות את **כל ארבעת יחסי הגומלין** עם הדוגמאות שהאיפיון נקב בהן (מקדמות = עצמאי, תשלום מע"מ = אחרי, ניכויים = מובלע, פנסיה = לפני-חוסם), הקישור שלהן לשירותים, וסינון סוג ישות על שתי תבניות המע"מ. **נתוני דוגמה** — התבניות האמיתיות ומה גורר מה הן הזנה של המשרד, לא מבנה.
