...
מחזיקה את כל אמצעי הקשר של לקוח (טלפונים, מיילים) — משמשת לזיהוי אוטומטי של פניות נכנסות ב-Omnichannel (וואטסאפ/מייל) ולשיוך שלהן ללקוח הנכון. מחליפה שדות "טלפון ראשי"/"מייל ראשי" קבועים על `clients` — היו תוצר של מגבלת אוריגמי (קושי להתאים לפי ערך בתוך קבוצה חוזרת). ב-Postgres אפשר לחפש ישירות מול הטבלה הזו, בלי שדה כפול קבוע, וגם לתמוך בכמה טלפונים/מיילים ללקוח.
