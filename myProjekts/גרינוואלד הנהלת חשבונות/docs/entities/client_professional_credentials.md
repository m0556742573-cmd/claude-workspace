...
מחזיקה פרטי גישה של הלקוח למערכות ממשלתיות (למשל מערכת מייצגים) — נדרש כדי שהמשרד יוכל לפעול בשם הלקוח מול הרשויות. מידע רגיש ביותר. נשארת טבלה נפרדת מ-`client_financial_credentials` (לא מוזגות) — לא בגלל מגבלת RLS-ברמת-שורה של אוריגמי (זה נפתר טבעית ב-Postgres), אלא מעיקרון **blast-radius אבטחתי**: הפרדת טבלאות מאפשרת לתת ל-service role עתידי גישה רק לטבלה אחת, ברמת מבנה — לא תלוי בנכונות של policy בודד.
