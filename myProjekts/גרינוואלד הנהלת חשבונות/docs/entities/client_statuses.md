...
לא ישות עצמאית במודל המקורי — lookup פנימי לשדה סטטוס של `clients`. נבחרה טבלה במקום Postgres enum כי לחלק מהסטטוסים יש **התנהגות שונה** (לא רק תווית): מושהה-בגין-חוב ממשיך גבייה, מוקפא-זמנית עוצר גבייה. `continues_collection` מאפשר לקוד/Views לבדוק דגל במקום להשוות מחרוזות קשיחות בכל מקום.
