...
- **View `active_clients`** — `select * from clients where deleted_at is null`, עם `security_invoker = true`. זהו נתיב הקריאה המומלץ, כדי שלא צריך לזכור את הסינון בכל שאילתה.
