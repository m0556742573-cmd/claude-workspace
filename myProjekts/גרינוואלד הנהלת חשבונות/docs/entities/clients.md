...
- **פתוח:** מחזור מע"מ / מחזור צפוי — יצחק עונה בנפרד (עמודה בודדת מול טבלה היסטורית).
- **פתוח:** "הערות כלליות" על הלקוח — כבר יש `client_notes` (מתועד, עם תוקף וכותב); לא ברור אם צריך גם שדה טקסט חופשי נפרד.
- **פתוח:** האם `client_contact_channels` (ערוצי קשר ברמת העסק) עדיין נחוץ עכשיו שיש `contact_channels` ברמת האדם, או שכל תקשורת עוברת דרך אנשי קשר ספציפיים.
