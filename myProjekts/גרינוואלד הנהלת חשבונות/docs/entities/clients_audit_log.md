...
מתעדת מי שינה מה ומתי בשדות קריטיים של לקוח (סטטוס, עובד אחראי, פרטי ישות) — כדי שתמיד יהיה אפשר לשחזר "מה היה נכון מתי". לא הייתה קיימת באוריגמי (נסמכו על Audit Log מובנה כללי). נוספה כאן במכוון — מענה ישיר למטרה 9 ב-`overview.md` ("תיעוד שמגן"): מול לקוח, מול ביקורת רשות המסים/ביטוח לאומי, ומול תביעת רשלנות.
