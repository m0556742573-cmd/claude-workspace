...
- **22 טבלאות בפועל על ה-DB החי**, כולן עם RLS מופעל (בלי policies עדיין — חסום כברירת מחדל ל-anon/authenticated):
...
לא הוסכם. אפשרויות: (א) מעבר לקבוצה ב' — שירותים/חובות (החלק הגדול והמורכב שנותר מתוך מודל 33 הישויות), או (ב) כתיבת RLS policies אמיתיים.
